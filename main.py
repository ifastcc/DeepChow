...
def analyze(name, sex, year, month, day, hour, minute, location, timezone, deepseek_api_key, gemini_api_key):
    ...
    for i in range(1, 8):
        ...
        try:
             ...
             result_gemini_json = {} # Use an empty dict to avoid KeyErrors later
        ...
        print(f"Finished section {i}/{7}. Time taken for this iteration: {elapsed_time_this_loop} seconds.")

        # Write results to file using the parsed JSON
        file.write("## " + prompt_gemini[2*i] + "\n")
        if prompt_gemini[2*i+1]: # Check if the list of sub-sections is not empty
            for j in range(len(prompt_gemini[2*i+1])):
                 section_title = prompt_gemini[2*i+1][j]
                 # Safely access the data from the parsed JSON
                 section_content = result_gemini_json.get(section_title, "Analysis not available.") # Use .get() for safety
    ...
    # Indicate success - maybe return the filename or a status code
    return 0
...
--------------------------------#-----------------------------------------------""")
    print("================================================================================")
    print("======================[WuLiang Tianzun Be With You.]============================")
    ...
    year = int(do_while_input("Your Birth Year [1900-2100]:", lambda x: x.isdigit() and 1900 <= int(x) <= 2100))

    month = int(do_while_input("Your Birth Month [1-12]:", lambda x: x.isdigit() and 1 <= int(x) <= 12))

    day = int(do_while_input("Your Birth Day [1-31]:", lambda x: x.isdigit() and 1 <= int(x) <= 31))

    hour = int(do_while_input("Your Birth Hour [0-23]:", lambda x: x.isdigit() and 0 <= int(x) <= 23))

    minute = int(do_while_input("Your Birth Minute [0-59]:", lambda x: x.isdigit() and 0 <= int(x) <= 59))

    location = do_while_input("Your Birth Location [e.g., AlbanyCounty,NewYork]:", lambda x: x.strip() != "")
    ...
    deepseek_key = do_while_input("Your DeepSeek API Key:", lambda x: x.strip() != "")
    gemini_key = do_while_input("Your Gemini API Key:", lambda x: x.strip() != "")
    ...
    print("\nTest analysis function finished at this dir")